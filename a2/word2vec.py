...
def negSamplingLossAndGradient(
    ...
    z = sigmoid(np.matmul(u_o.T, centerWordVec))
    loss = -np.log(z)
    ...
    for i in range(K):
        ...
        z = sigmoid(-np.matmul(u_k, centerWordVec))
        loss -= np.log(z)
        gradCenterVec += (1-z)*u_k
        gradOutsideVecs[neg_id] += (1-z)*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterWordVector = centerWordVectors[centerWordIdx]
    for word in outsideWords:
        currentOutsideWordVectorsIdx = word2Ind[word]
        current_loss, current_gradCenterVec, current_gradOutsideVecs = word2vecLossAndGradient(currentCenterWordVector, currentOutsideWordVectorsIdx, outsideVectors=outsideVectors, dataset=dataset)
        loss += current_loss
